	...
	class slimdaxo_htaccess {
		...
		private static function writefile()
		{
		    ...
		    if ($func == 'htaccess' && $type != '' ) {
				if($type == 1) {
					rex_file::copy(rex_path::addon('slimdaxo', 'setup/.htaccess'), rex_path::frontend('.htaccess'));
				...
				if($type == 3) {
					...
					if($file) {
						preg_match_all("/(#start:slimdaxo)\s*(.*)\s*(#end:slimdaxo)/", $file, $output_arr);
						if(is_array($output_arr) && empty($output_arr[0])) {
							$search = '# REWRITE RULE FOR SEO FRIENDLY IMAGE MANAGER URLS';
							$content = "\n";
							$content .= '#start:slimdaxo'."\n";
							$content .= 'RewriteRule ^slimdaxo/(.*)$ %{ENV:BASE}/index.php?rex-api-call=slimdaxo&%{QUERY_STRING} [QSA,L]'."\n";
							$content .= '#end:slimdaxo'."\n";
